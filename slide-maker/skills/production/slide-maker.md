> **Cross-reference**: Original source at `~/.claude/skills/slide-maker/SKILL.md`
> **Category**: Production | **Shared standards**: [quality-standards.md](../shared/quality-standards.md)
...
## Mobile Compatibility (HTML Slides)

> **適用於 Web 版投影片 (HTML)**

當產出 HTML 格式投影片時，必須考慮 Mobile 相容性，確保在手機和平板上也能正常觀看。
...
HTML 投影片通常設計給桌面螢幕（如 1920x1080），在手機上需要縮放處理：

**方案 A：CSS Transform 縮放（推薦）**
...
```html
<!-- 控制列放在 footer 位置 -->
<div class="zoom-controls" style="position: fixed; bottom: 0; left: 0; right: 0;">
  <button onclick="zoomOut()">−</button>
  <span id="zoom-level">100%</span>
  <button onclick="zoomIn()">+</button>
  <button onclick="fitToScreen()">適應</button>
...
function applyScale() {
  document.querySelector('.slide-container').style.transform = `scale(${currentScale})`;
...
- [ ] **iPhone Safari (iOS 15+) 顯示正常**
- [ ] **Android Chrome 顯示正常**
- [ ] **直向模式內容不被裁切**
- [ ] **橫向模式內容完整顯示**
- [ ] **縮放控制元件位於可見位置（建議 footer）**
- [ ] **觸控按鈕足夠大（≥44x44px）**
- [ ] **字體縮放後仍可閱讀**
